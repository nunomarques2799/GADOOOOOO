...
- **Atualizações**: duas vias, um só banner (`BannerAtualizacao` → `useAtualizacao()`). Desktop = instalador Electron; telemóvel = **EAS Update** (`eas update --branch preview`), que entrega **só alterações de JS**. Mexer em código nativo — plugin novo no `app.json`, permissão, dependência nativa — continua a exigir `eas build`. Na dúvida: se editou `app.json`/`package.json`, é build; se só editou `src/`, chega o update.
