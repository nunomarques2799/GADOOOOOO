...
| 35 | `schema_chat.sql` | Conversas: um grupo por exploração (dono e trabalhadores, sem o veterinário), mensagens privadas entre quem partilha uma exploração, denunciar e bloquear, e a limpeza dos seis meses por `pg_cron`. Vem depois do lint pela mesma razão do 34, e é o único ficheiro cujas políticas **não** têm `eh_superadmin()`: as conversas dos clientes não se leem. | **34** (é o último), 2, **15** (o prazo do veterinário), **6** (a suspensão congela também as conversas) |
...
| 40 | `schema_sociedade.sql` | Sociedades agrícolas: o papel `supervisor` (cria explorações, supervisiona-as e não mexe no gado) e o `admin` a poder entrar por convite como LÍDER de exploração. `perfil.tipo_conta` decide qual dos dois nasce. Muda quem paga (`exploracao_ativa`), quem é convidado (`eh_convidado`) e quem mexe na equipa. **É o primeiro ficheiro com `alter type ... add value`** — ver a regra do `::text` no cabeçalho dele. | **todos**, e em especial **2** (os papéis), **6** (a suspensão), **13** (o `pode_cap`), **15** (o prazo), **20** (`eh_convidado`), **23** (a agenda), **32** (o `criar_convite`), **33** (o lint) |
